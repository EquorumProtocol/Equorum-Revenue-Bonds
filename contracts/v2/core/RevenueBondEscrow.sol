...
contract RevenueBondEscrow is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public constant SALE_FEE_BPS = 200;  // 2% fee on sales
    ...
    /**
     * @notice Start token sale (protocol only)
     * @param _pricePerToken Price per token in wei
     * @param _treasury Address to receive protocol fees
     */
    function startSale(uint256 _pricePerToken, address _treasury) external {
        require(msg.sender == protocol, "Only protocol");
        require(state == SeriesState.Active, "Not active");
        require(_pricePerToken > 0, "Invalid price");
        require(_treasury != address(0), "Invalid treasury");
        require(balanceOf(protocol) > 0, "No tokens to sell");
        
        tokenPriceWei = _pricePerToken;
        treasury = _treasury;
        saleActive = true;
        
        emit SaleStarted(_pricePerToken);
    }
    
    /**
     * @notice Stop token sale (protocol only)
     */
    ...
    function buyTokens(uint256 tokenAmount) external payable nonReentrant {
        ...
        uint256 protocolBalance = balanceOf(protocol);
        require(protocolBalance >= tokenAmount, "Insufficient tokens for sale");
        
        // Calculate cost
        uint256 totalCost = (tokenAmount * tokenPriceWei) / 1e18;
        require(totalCost > 0, "Cost too low");
        require(msg.value >= totalCost, "Insufficient ETH");
        
        // Calculate fee (2%)
        uint256 fee = (totalCost * SALE_FEE_BPS) / 10000;
        uint256 protocolReceives = totalCost - fee;
        
        // Transfer tokens from protocol to buyer
        _update(protocol, msg.sender, tokenAmount);
        
        // Send ETH to protocol (minus fee)
        (bool successProtocol, ) = protocol.call{value: protocolReceives}("");
        require(successProtocol, "Protocol payment failed");
        ...
        if (msg.value > totalCost) {
            (bool successRefund, ) = msg.sender.call{value: msg.value - totalCost}("");
            require(successRefund, "Refund failed");
        }
        
        emit TokensPurchased(msg.sender, tokenAmount, totalCost, fee);
    }
     ...
     * @param tokenAmount Amount of tokens to buy
     */
    function calculateBuyCost(uint256 tokenAmount) external view returns (uint256 totalCost, uint256 fee) {
        if (!saleActive || tokenPriceWei == 0) return (0, 0);
        totalCost = (tokenAmount * tokenPriceWei) / 1e18;
        fee = (totalCost * SALE_FEE_BPS) / 10000;
    }
    ...
    function distributeRevenue() external payable {
        ...
        try IProtocolReputationRegistry(reputationRegistry).recordDistribution(protocol, msg.value) {
        } catch {
    ...
    function declareDefault() external {
        ...
        // Report default to reputation registry
        try IProtocolReputationRegistry(reputationRegistry).reportDefault(protocol, "Failed to deposit principal") {
        } catch {
    ...
    function getBondInfo() external view returns (
        ...
        uint256 remaining = principalAmount > totalPrincipalClaimed 
            ? principalAmount - totalPrincipalClaimed 
            : 0;
        return (
            protocol,
    ...
    /**
     * @notice Get series info (compatible with RevenueRouter)
     */
    function getSeriesInfo() external view returns (
        ...
        bool seriesActive = (state == SeriesState.Active) && block.timestamp < maturityDate;
        uint256 remaining = block.timestamp < maturityDate ? maturityDate - block.timestamp : 0;
        return (
            protocol,
            revenueShareBPS,
            maturityDate,
    ...
    function _update(address from, address to, uint256 value) internal virtual override {
        // Enforce minimum purchase (except for burns and mints)
        if (to != address(0) && from != address(0)) {
            // Check recipient will have minimum (except protocol)
            if (to != protocol) {
                require(balanceOf(to) + value >= minPurchaseAmount, "Recipient below minimum");
            }
            
            // Check sender will have minimum OR zero (full exit allowed) (except protocol)
            if (from != protocol) {
