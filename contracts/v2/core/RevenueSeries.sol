...
contract RevenueSeries is ERC20, Ownable, ReentrancyGuard {
    ...
    address public immutable reputationRegistry; // Reputation registry for tracking
    ...
    uint256 public immutable minDistributionAmount; // Minimum distribution to prevent griefing
     ...
     * @param _minDistributionAmount Minimum distribution amount
     */
    constructor(
        string memory _name,
    ...
    function distributeRevenue() external payable {
        ...
        // Protect against rounding to zero
        ...
        // Record distribution in reputation registry (graceful - don't block distribution if registry fails)
        try IProtocolReputationRegistry(reputationRegistry).recordDistribution(protocol, msg.value) {
            // Success - reputation updated
        } catch {
            // Registry failed - distribution still proceeds
            // This ensures bondholders always get paid even if registry has issues
        }
    ...
    function _claimFor(address user) internal {
        ...
        // Zero rewards BEFORE external call to prevent reentrancy
        rewards[user] = 0;
    ...
    /**
     * @notice Calculate effective minimum distribution based on current supply
     * @dev Returns the minimum amount needed to avoid rounding to zero
     * This helps protocols understand the real minimum they should send
     * @return effectiveMin Minimum distribution amount that won't round to zero
     */
    function getEffectiveMinDistribution() external view returns (uint256 effectiveMin) {
        uint256 supply = totalSupply();
        if (supply == 0) return 0;
        
        // Calculate minimum to ensure revenuePerToken > 0
        // revenuePerToken = (amount * 1e18) / supply
        // For revenuePerToken >= 1: amount >= ceil(supply / 1e18)
        effectiveMin = (supply + 1e18 - 1) / 1e18; // Ceiling division
